            ...
            If My.Computer.FileSystem.FileExists(System.AppDomain.CurrentDomain.BaseDirectory & "\ClassPrerequisites.txt") = True Then
                ...
                Do While objReader.Peek <> -1
                    ...
                    For i = 1 To strSplitReader.Length - 1 Step 3
                        ...
                        ElseIf strSplitReader(i) = "customer" Then
...
Namespace Character
    Public Class PlayerCharacter
        Public strName As String = ""
        Public colBackground As Collection 'used for any character definition, such as race, profession, or astrological sign
        Public intExperience As Integer = 0
        Public intlevel As Integer = 0
        Public colAbilities As Collection
        Public colAttacks As Collection 'used for any character attacks, melee, ranged, or magical includes bonus to attack and any stat modifiers
        'Public intArmorCheckPenalty As Integer = 0
        Public colStats As Collection 'used for any stats.  Has a derivation level - a 0 level stat is not derived from any other stat, a 1st level stat is derived from one or more base stats, and 2nd is derived from one or more 0 and 1st level stats, and so on.
        Public colEquipmentSlots As Collection
        Public colItems As Collection 'Items carried includes: Item name, Equipment slot (if Equiped)
        Private colClasses As New Collection 'format: key = class, item = level
        Public strSavePath As String
        Public Sub New()
            colClasses = New Collection
            colBackground = New Collection
            colAttacks = New Collection
            colStats = New Collection
            colEquipmentSlots = New Collection
            colItems = New Collection
            colAbilities = New Collection
        End Sub
        Public Function chain(ByVal strKey As String, ByVal strfilename As String, ByVal bolErrorMsg As Boolean) As String
            Dim strCurrentSearchItem As String = ""
            Dim strArray(0) As String
            ...
            Return strCurrentSearchItem
        End Function
        Public Sub AddAbility(ByVal strAbility As String, ByVal strType As String)
            'need to get a file that adds schools to spell focus or weapons to weapon focus
            colAbilities.Add(strAbility + "," + strType, strAbility)
        End Sub
        Public Function GetFeatCollection() As Collection
            Return colFeats
        End Function
